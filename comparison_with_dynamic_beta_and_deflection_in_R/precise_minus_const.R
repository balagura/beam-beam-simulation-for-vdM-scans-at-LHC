...
new.lumi.w.over.wo <- {
    ...
    .[,list(separation = x2,
            ...
            vdm.profile,
            orbit = orbitRelLumiChange(x2,0,sig,N,N,beta,Qx,Qy))]
}
...
gg.cor.orbit.subtr.and.added <- function() {
    ...
    combined <- rbind(old,
                  new.lumi.w.over.wo.precise[, list(separation,
                                                    variable = 'new.orbit.not.subtr',
                                                    value = cor)],
                  new.lumi.w.over.wo[, list(separation,
                                            variable = 'new.orbit.subtr',
                                            value = cor)],
                  new.lumi.w.over.wo[, list(separation,
                                            variable = 'new.orbit.subtr.and.added',
                                            value = cor*orbit)])
    ...
    ggplot() +
        ...
        scale_colour_manual(values = c(new.orbit.not.subtr='black',
                                       new.orbit.subtr.and.added='#808080',
                                       new.orbit.subtr='#009900',
...
save.plot('gg.cor.orbit.subtr.and.added')

print('Cross-section corrections: Xsec(beam-beam)/Xsec(NO beam-beam)')
print(xsec)
print('New cross-section without orbit subtraction: Xsec(beam-beam)/Xsec(NO beam-beam)')
xsec.precise$new
print('New cross-section, without orbit subtraction - (with subtraction + orbit): Xsec(beam-beam)/Xsec(NO beam-beam)')
